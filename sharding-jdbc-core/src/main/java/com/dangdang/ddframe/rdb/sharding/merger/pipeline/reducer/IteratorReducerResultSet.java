...
/* 迭代归并的聚集结果集, 没有order和groupBy的情况下 逐个返回ResultSet */
public final class IteratorReducerResultSet extends AbstractDelegateResultSet {
    private int resultSetIndex = 1;
    
    public IteratorReducerResultSet(final ResultSetMergeContext resultSetMergeContext) throws SQLException { super(resultSetMergeContext.getShardingResultSets().getResultSets()); }
    
    @Override
    protected boolean firstNext() throws SQLException { return processCurrent(); }
    @Override
    protected boolean afterFirstNext() throws SQLException {return processCurrent() || (!isOutOfIndex() && processNext()); }
    private boolean processCurrent() throws SQLException { return getDelegate().next(); }
